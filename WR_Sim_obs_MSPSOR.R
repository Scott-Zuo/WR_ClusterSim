...
################################################################
####### Expanded with interaction terms (Observational) ########
################################################################
...
scenarios <- read.table("WR_Params_obs.txt", header=TRUE, sep="")
...
# covariates
x1 <- rnorm(n_total, mean = 1, sd = sds[1])
x2 <- rnorm(n_total, mean = 0.9, sd = sds[2])
x3 <- rnorm(n_total, mean = 0.8, sd = sds[3])
x4 <- rbinom(n_total, 1, bern_param[1])
x5 <- rbinom(n_total, 1, bern_param[2])
x6 <- rbinom(n_total, 1, bern_param[3])

cov_all <- data.frame(x1, x2, x3, x4, x5, x6)




#############################################################################
###############True Propensity Score for Treatment Assignment################
#############################################################################


# lambda values
lambda0 <- -0.5
lambda <- c(0.5, -0.4, 0.3, -0.6, 0.5, -0.3)

linear_terms <- lambda[1]*x1 + lambda[2]*x2 + lambda[3]*x3 + 
  lambda[4]*x4 + lambda[5]*x5 + lambda[6]*x6
#quadratic_terms <- 2*lambda[1]*(x1^2) + 2*lambda[2]*(x2^2) + 2*lambda[3]*(x3^2) + 
#  2*lambda[4]*(x4^2) + 2*lambda[5]*(x5^2) + 2*lambda[6]*(x6^2)


# Calculate the propensity score using the logit link
logit_prop_score <- lambda0 + linear_terms
#logit_prop_score <- lambda0 + linear_terms + quadratic_terms
true_propensity_score <- inv_logit(logit_prop_score)

# Assign treatment based on the propensity score
treatment_assignment <- rbinom(n_total, 1, true_propensity_score)
...
# Split the data into treatment and control based on the treatment assignment
trt_cov <- cov_all[treatment_assignment == 1, ]
ctrl_cov <- cov_all[treatment_assignment == 0, ]


n_count_trt <- nrow(trt_cov)
n_count_ctrl <- nrow(ctrl_cov)
