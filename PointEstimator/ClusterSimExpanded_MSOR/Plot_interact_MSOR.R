...
step_size <- 60
re_count <- 10
#setwd("/Users/sizhezuo/Desktop/YCASResearch/WR_Cluster/ClusterSimExpanded_MSOR/results_interact_MSOR/results_0224")
...
png(filename = "fig_interactMSOR_Var_WD.png", units = "in",
    width = 6, height = 4, res = 300)
var_plot_WD
dev.off()

png(filename = "fig_interactMSOR_Var_WR.png", units = "in",
    width = 6, height = 4, res = 300)
var_plot_WR
dev.off()
...
png(filename = "fig_interactMSOR_MSE_WD.png", units = "in",
    width = 6, height = 4, res = 300)
mse_plot_WD
dev.off()

png(filename = "fig_interactMSOR_MSE_WR.png", units = "in",
    width = 6, height = 4, res = 300)
mse_plot_WR
dev.off()
...
bias_plot_WD <- ggplot(sim_df_WD, aes(x = samplesize, y = abs(bias),
  ...
  xlab("Sample Size") + ylab("Percentage of Bias") +
  scale_x_continuous(breaks=seq(0,step_size*re_count,step_size)) +
  theme(
    plot.title = element_text(color="red", size=12, face="bold.italic"),
    axis.title.x = element_text(size=12),
...
png(filename = "fig_interactMSOR_Bias_WD.png", units = "in",
    width = 6, height = 4, res = 300)
bias_plot_WD
dev.off()

png(filename = "fig_interactMSOR_Bias_WR.png", units = "in",
    width = 6, height = 4, res = 300)
bias_plot_WR
dev.off()
...
#––– interact_MSOR: Relative Efficiency Plot for Win Difference (RE_WD) –––
re_plot_WD <- ggplot(re_df_WD, aes(x = samplesize, y = relative_eff, color = estimator)) +
  geom_point() +
  geom_line(aes(group = estimator)) +
  # distinct colors: black (IPW), blue (OW), green (AIPW), red (AOW)
...
png(filename = "fig_interactMSOR_RE_WD.png", units = "in", width = 6, height = 4, res = 300)
re_plot_WD
dev.off()

ggsave("fig_interactMSOR_RE_WD.eps", plot = re_plot_WD, device = "eps", width = 6, height = 4)


#––– interact_MSOR: Relative Efficiency Plot for Win Ratio (RE_WR) –––
...
png(filename = "fig_interactMSOR_RE_WR.png", units = "in", width = 6, height = 4, res = 300)
re_plot_WR
dev.off()


ggsave("fig_interactMSOR_RE_WR.eps", plot = re_plot_WR, device = "eps", width = 6, height = 4)
